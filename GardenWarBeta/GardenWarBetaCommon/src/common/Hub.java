 ...
 * Clients are defined by subclasses of the class Client.
...
public class Hub {
    ...
    synchronized public boolean sendToOne(int recipientID, Object message) {
        ...
        if ( ! (message instanceof Serializable) )
            throw new IllegalArgumentException("Messages must implement the Serializable interface.");
        //find the corresponding ConnectionToClient Object in the treeMap and send the message
    ...
    synchronized private void acceptConnection(ConnectionToClient newConnection) {
        //load the newConnection information bundle to the treeMap of the Hub
    ...
    private class ServerThread extends Thread {  // Listens for connection requests from clients.
        ...
        public void run() {
            try {
                while ( ! shutdown ) {
                    //accepts new client as long as condition satisfied
                    Socket connection = serverSocket.accept();
                    if (shutdown) {
                        System.out.println("Listener socket has shut down.");
                        break;
                    }
                    //instantiate a ConnectionToClient for each new client
    ...
    private class ConnectionToClient { // Handles communication with one client.
        ...
        //incomingMessages is passed by the Hub's incomingMessage
        //meaning each ConnectionToClient instance will use this queue
        //which is why this has to be a blocking queue
        ...
        private class SendThread extends Thread {
            public void run() {
                try {
                    //initialization
                    //creates the ObjectOutputStream from the connection socket
                    out = new ObjectOutputStream(connection.getOutputStream());
                    //creates the ObjectInputStream from the connection socket
                    ...
                    //start to listen to player actions
                    receiveThread.start();
                }
                catch (Exception e) {
                    try {
                ...
                //Send all the messages from the outgoingMessages Queue
                try {
